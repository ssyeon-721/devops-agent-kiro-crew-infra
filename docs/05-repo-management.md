...
## 2.5 오해 주의 — 편집은 여전히 로컬에서 한다

이 방식을 "이제 GitHub에서 코드를 수정한다"로 오해하기 쉬운데, **그렇지 않다.**

바뀐 것은 **수정 위치**가 아니라 **커밋이 나가는 목적지**뿐이다.

| | 이전 | 지금 |
|--|------|------|
| 코드 편집 장소 | 로컬 워크스페이스 | 로컬 워크스페이스 (동일) |
| app 코드를 올리는 법 | 임시 복사본(/tmp)에 복사 후 푸시 | app 폴더에서 바로 푸시 |

즉:
- 파일 편집은 예전처럼 **내 컴퓨터의 워크스페이스**에서 에디터로 한다. GitHub 웹에서 고치는 게 아니다.
- 달라진 건, 로컬에서 고친 걸 push할 때 **어느 폴더에서 push하느냐**에 따라 다른 레포로 나간다는 것뿐이다.

```
로컬에서 편집 (항상 여기서)
     │
     ├── 루트에서 git push        → devops-agent-kiro-crew-infra 레포
     │   (docs, terraform)
     │
     └── app 폴더에서 git push    → poc-eks-incident-app 레포
         (main.py, Dockerfile 등)
```

비유하면 작업대(로컬)는 그대로이고, 택배를 부치는 창고(GitHub 레포)만 물건 종류에 따라 둘로 나뉜 것이다. 예전엔 app 물건을 부치려고 임시 상자(/tmp)에 옮겨 담아야 했는데, 이제는 app 폴더에서 바로 부칠 수 있게 된 것이 유일한 변화다.
