...
- [ ] AWS 검증 계정 분리 확인
- [ ] ap-northeast-2 리전 기본 한도 확인 (EKS, EC2, EIP 등)
- [ ] state용 S3 버킷 수동 생성
  ```bash
...
- [ ] GitHub OIDC 신뢰 역할 구성
  - IAM → Identity providers → `token.actions.githubusercontent.com` 등록
  - `github-actions-terraform` 역할 생성, 신뢰 정책에 레포·브랜치 한정

### 1-2. Terraform 모듈 구성

- [ ] `backend.tf` 작성 및 `terraform init`
- [ ] `modules/network` apply
  - VPC, 퍼블릭/프라이빗 서브넷
  - **워커 노드 서브넷: /28** (시나리오 6 IP 고갈 필수 조건)
  - NAT Gateway, 라우팅 테이블
- [ ] `modules/eks` apply
  - EKS 1.31+, 관리형 노드그룹 2대 (t3.large)
  - EKS Pod Identity Agent 애드온
  - `kubectl` 접속 확인
- [ ] `modules/operator-iam` apply
  - `devops-agent-operator-policy` 생성
  - `devops-agent-operator-role` 생성 + Pod Identity Association
- [ ] `modules/storage` apply
  - 인시던트 S3 버킷 (버전관리, 30일 라이프사이클)
  - CloudWatch 로그그룹 `cw-log-group-devops-agent-operator` (14일 보존)
- [ ] `modules/registry` apply
  - ECR 레포: `poc-operator`, `poc-app`
  - 라이프사이클 정책 (이미지 30개 보관)
- [ ] `modules/crew-host` apply
  - EC2 t3a.medium, AL2023
  - 보안그룹 (인바운드 0, SSM 접속용)
...
- [ ] Dockerfile 작성
- [ ] GitHub Actions 빌드 CI 구성 (app 레포)
  - PR: 이미지 빌드 + 태그
  - main 머지: ECR 푸시 (`latest` + 커밋 SHA 태그)
- [ ] 정상 상태 Kubernetes 매니페스트 작성
  - Deployment (requests/limits 정상값)
  - Service
  - readinessProbe: `/healthz`
- [ ] ECR 푸시 확인, 클러스터에 정상 배포 확인
...
- [ ] `kubectl get nodes` → 2대 Ready
- [ ] 테스트 워크로드 Pod Running 확인
- [ ] `GET /healthz` 200 응답 확인
- [ ] `reset.sh` 실행 후 정상 상태 복원 확인
